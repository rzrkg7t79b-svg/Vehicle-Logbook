...
function StreakVan({ streak, imgW, imgH, labelSize, numSize, glowSize }: {
  ...
  const fontSize = digitCount >= 4 ? numSize * 0.47 : digitCount === 3 ? numSize * 0.65 : digitCount === 2 ? numSize * 0.85 : numSize;
...
export function DailyStreak({ streak }: DailyStreakProps) {
  return (
    <div data-testid="daily-streak">
      <StreakVan streak={streak} imgW={180} imgH={100} labelSize={11} numSize={34} glowSize={6} />
    </div>
  );
}
